...
VLM pool: 10 total VLMs, 5 randomly sampled per evaluation (without replacement).
VLM_WEIGHTS is a fixed (10, 5) matrix, seeded at 42 — same every run.
Noise σ = 0.1 — small enough that signal dominates, large enough for HT to work.
"""
...
NUM_RATERS = 5      # How many VLMs score each image (sampled without replacement)
NOISE_SIGMA = 0.1   # Stochasticity per query (models real VLM inter-query variance)
...
# L2-normalised so all VLMs produce scores on a comparable scale.
_rng = np.random.RandomState(42)
_raw_weights = _rng.randn(NUM_VLMS, FEATURE_DIM).astype(np.float32)
_norms = np.linalg.norm(_raw_weights, axis=1, keepdims=True) + 1e-8
VLM_WEIGHTS = _raw_weights / _norms          # shape: (10, 5),  fixed forever
...
def torch_vlm_scorer(images_numpy: np.ndarray) -> np.ndarray:
    ...
    For each image:
      ...
      - Randomly sample NUM_RATERS VLMs from the pool (different each call —
        this is intentional: it simulates querying different VLMs each time)
    ...
    with torch.no_grad():
        ...
        # Randomly pick NUM_RATERS VLMs (same subset for all images in batch)
        selected_indices = torch.randperm(NUM_VLMS)[:NUM_RATERS]  # (5,)
        selected_scores  = all_scores[:, selected_indices]         # (batch, 5)
...
if __name__ == "__main__":
    ...
    print(f"Bright beats dark in {wins}/50 calls (expect ~40-50 if any VLMs prefer brightness)")
    print("\n=== All checks passed ===")