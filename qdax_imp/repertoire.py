...
- Add exploration window for initial bootstrapping
"""
...
class DistributionalRepertoire(MapElitesRepertoire):
    ...
    def add(
        ...
        exploration_iter: Optional[int] = None,  # NEW: Support exploration window
    ) -> "DistributionalRepertoire":
        """
        ...
        NEW: exploration_iter parameter enables exploration window bypass.
        If exploration_iter < 50, the HT gate is bypassed and naive mean-based
        replacement is used to bootstrap the archive.
        """
        ...
        def _update_candidate(repertoire, inputs):
            ...
            # NEW: Exploration window bypass
            # During first 50 iterations, use naive mean-based replacement
            # This allows the archive to bootstrap with diverse candidates
            if exploration_iter is not None and exploration_iter < 50:
            ...
            # Normal HT gate logic (after exploration window)
            if bool(use_ht):
