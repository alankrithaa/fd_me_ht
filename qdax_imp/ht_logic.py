...
def calculate_ht_replacement(new_scores, old_scores, alpha=0.05, delta_min=0.6):
    ...
    FIXED: Now uses correct two-sample z-test with standard error of the difference.
    ...
    # FIXED: Use correct two-sample z-test with standard error of the difference
    mean_new = jnp.mean(new_scores)
    mean_old = jnp.mean(old_scores)
    
    # Pooled variance for two-sample z-test
    std_new = jnp.std(new_scores)
    std_old = jnp.std(old_scores)
    pooled_var = (std_new**2 + std_old**2) / 2.0 + 1e-8
    
    # CRITICAL FIX: Standard error of the difference between means
    # SE(μ_new - μ_old) = sqrt(σ²/M + σ²/M) = sqrt(2σ²/M)
    M = new_scores.shape[0]
    se = jnp.sqrt(pooled_var / M + pooled_var / M)  # Standard error, not pooled std
    
    # Z-score using standard error
    z_score = jnp.abs(mean_new - mean_old) / (se + 1e-10)  # Extra epsilon for safety
